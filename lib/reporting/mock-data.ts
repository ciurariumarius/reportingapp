...
export function metaReport(client: ClientConfig, range: DateRange) {
  ...
  const daily = listDates(range).map((date, index) => {
    ...
    return {
      ...
      cost_per_landing_page_view: calcCpa(spend, landingPageViews),
  ...
  const campaigns = [
    ...
    return {
      ...
      cost_per_landing_page_view: calcCpa(campaignSpend, campaignLandingPageViews),
  ...
  const kpis: PlatformKpis = {
    ...
    costPerLandingPageView: calcCpa(spend, landingPageViewsTotal),
    ctr: calcCtr(clicks, impressions),
    cpc: calcCpc(spend, clicks),
